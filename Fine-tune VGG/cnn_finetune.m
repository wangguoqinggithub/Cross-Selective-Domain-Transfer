...
%find lr%
indx = find(cellfun(@(l) isfield(l,'learningRate'),net.layers)); 
lr = cellfun(@(l) l.learningRate, net.layers(trainable_layers(indx)),'UniformOutput',false);
%find lr%
layers_for_update = {trainable_layers(end), fc_layers, trainable_layers}; 
% ind = cellfun(@(l) isfield(l,'learningRate'),net.layers); 
